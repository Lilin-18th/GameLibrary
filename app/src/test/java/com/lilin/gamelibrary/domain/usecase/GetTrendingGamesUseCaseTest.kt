...
class GetTrendingGamesUseCaseTest {
    private lateinit var gameRepository: GameRepository
    private lateinit var useCase: GetTrendingGamesUseCase

    @BeforeEach
    fun setUp() {
        gameRepository = mockk()
        useCase = GetTrendingGamesUseCase(gameRepository)
    }
    ...
    fun test_GetTrendingGamesUseCase_success() = runTest {
        ...
        val response = useCase(1, 10)

        assertTrue(response.isSuccess)
        assertEquals(4, response.getOrNull()?.size)
        coVerify(exactly = 1) {
            gameRepository.getTrendingGames(1, 10)
        }
    }
